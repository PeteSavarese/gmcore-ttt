...
function meta:WantsToDM()
	...
	if allowed then
		if self:IsActive() then
			gmcore.chatprint(self, "You can't enter spectator deathmatch when you're alive.")
		elseif GetRoundState() != ROUND_ACTIVE then
			gmcore.chatprint(self, "Current round is inactive.")
		elseif self.curSlayRound then
			gmcore.chatprint(self, "You can't enter spectator deathmatch while slain.")
		elseif not self.spawning_ghost then
			if tonumber(self.DMTimer) and self.DMTimer > 0 then
				gmcore.chatprint(self, "Wait " .. tostring(self.DMTimer) .. " second(s) before using this command again!")
			else
				local self = self
		...
		gmcore.chatprint(self, "You are not allowed to enter Spectator Deathmatch")
	end
end
...
hook.Add("EntityTakeDamage", "EntityTakeDamage_SpecDMHitmarker", function(ent, dmginfo)
	if not ent:IsPlayer() or not ent:IsGhost() then return end
...
hook.Add("DoPlayerDeath", "DoPlayerDeath_SpecDMRagdoll", function(ply)
	if ply:IsGhost() then ply:CreateRagdoll() end
end)
