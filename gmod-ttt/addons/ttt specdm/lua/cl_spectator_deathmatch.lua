...
hook.Add("Initialize", "Initialize_Ghost", function()
	...
	function util.TraceLine(tbl)
		...
		if not IsValid(localPly) or not localPly.IsGhost then
			-- Spray and other things that use TraceLine without a valid player or before the player is initialized should not error
			return oldTraceLine(tbl)
		end

		local plyghost = localPly:IsGhost()
...
hook.Add("TTTSettingsTabs", "SpecDM_TTTSettingsTab", function(dtabs)
	...
	dtabs:AddSheet("Spectator Deathmatch", dsettings, "icon16/gun.png", false, false,
		"Spectator deathmatch related settings")
end)
...
hook.Add("HUDPaint", "HUDPaint_SpecDM", function()
	...
	if autorespawntime ~= -2 then
		if SpecDM.AutomaticRespawnTime > 0 then
			...
			draw.DrawText(
			"Press a key to respawn!\nYou will be automaticly respawned in " ..
			rtime .. " second" .. (rtime ~= 1 and "s" or ""), "Trebuchet24", x, y, COLOR_WHITE, TEXT_ALIGN_CENTER)
		else
	...
	elseif respawntime ~= -2 then
		...
		if waittime > -1 then
			draw.DrawText("You need to wait " .. waittime .. " second(s) before you can respawn", "Trebuchet24", x, y,
