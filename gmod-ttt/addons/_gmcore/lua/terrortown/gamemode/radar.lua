---Integrates Pointshop upgrade_radar to reduce scan cooldown.

-- should mirror client
local iDefaultChargeTime = 30

local math = math

---@param ply Player The player who initiated the scan
...
local function RadarScan(ply, cmd, args)
	if IsValid(ply) and ply:IsTerror() then
		if ply:HasEquipmentItem(EQUIP_RADAR) then
				...
				if ply:PS_HasItemEquipped("upgrade_radar") then
					ply.radar_charge =  CurTime() + iDefaultChargeTime - (4 * ply:PS_GetUpgradeLevel("upgrade_radar")) -- Decrease by 4 seconds per level
				else
					ply.radar_charge =  CurTime() + iDefaultChargeTime
				end
