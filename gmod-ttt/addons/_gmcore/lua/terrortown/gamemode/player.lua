---Override of TTT player spawning, dying, and round lifecycle logic.
---for deathcards, spectating, and loadouts.
...
---@param ply Player The player who just connected
...
---@param ply Player The player who is spawning
...
---@param ply Player The player to check suitability for
---@param spwn Entity|Vector The spawn entity or jury-rigged position
---@param force boolean If true, kill blocking players
---@param rigged? boolean If true, `spwn` is a Vector rather than an entity
---@return boolean suitable Whether the spawnpoint is usable
...
---@param shuffled boolean Shuffle the resulting table
---@param force_all boolean Include info_player_start entities as fallback
---@return Entity[] spawns Table of valid spawn entities
...
---@param ply Player The player who needs a spawn
---@return Entity spawnpoint The chosen spawn entity
...
function GM:KeyPress(ply, key)
	...
	local hasDeathCard = gmcore.Deathcard.PanelStates[ply]
	...
	if ply:IsSpec() and not ply:GetRagdollSpec() then
			...
			if key == IN_ATTACK then
				if hasDeathCard then
					-- If we have deathcard open, tell client to exit
					ply:SendLua([[gmcore.Deathcard:PlayerExitCard()]])
			...
			elseif key == IN_JUMP then
				-- unstuck if you're on a ladder etc
...
function GM:PlayerDeath(victim, infl, attacker)
	...
	if attacker:IsPlayer() and attacker != victim then
		spectateEnt = attacker:Alive() and attacker or attacker:GetNWEntity("ttt_ragdoll")
	else
		-- Suicide so spectate our corpse
		spectateEnt = victim:GetNWEntity("ttt_ragdoll")
	end

	victim:SetTeam(TEAM_SPEC)
	victim:Freeze(false)
	victim:Spectate(OBS_MODE_CHASE)
	victim:SpectateEntity(spectateEnt)
...
function GM:EntityTakeDamage(ent, dmginfo)
	if not IsValid(ent) or ent.isPoltergeisted then return end
	local att = dmginfo:GetAttacker()

	-- Block damage from spectator-controlled props (but not during spectator deathmatch)
	if ent:IsPlayer() and dmginfo:IsDamageType(DMG_CRUSH) and not (ent.IsGhost and ent:IsGhost()) then
			local spec = PROPSPEC.IsSpectatorDmg(dmginfo)
			if IsValid(spec) then
				-- Completely suppress the damage event - scale to 0 to prevent karma/logging
...
function GM:PlayerTakeDamage(ent, infl, att, amount, dmginfo)
	if ent.isPoltergeisted then return end
	...
	if ent.ignite_info and dmginfo:IsDamageType(DMG_DIRECT) then
			...
			if (not IsValid(datt)) or (not datt:IsPlayer()) then
				...
				if IsValid(ignite.att) and IsValid(ignite.infl) then
						...
						local upgradeFlareLevel = ignite.att:PS_GetUpgradeLevel("upgrade_enhanced_pyrotechnics")

						if ignite.att:PS_HasItemEquipped("upgrade_enhanced_pyrotechnics") and upgradeFlareLevel > 2 then
							dmginfo:SetDamage(dmginfo:GetDamage() + ((upgradeFlareLevel - 2) * 0.25))
						else
	...
	if ent != att and IsValid(att) and att:IsPlayer() and GetRoundState() == ROUND_ACTIVE and math.floor(dmginfo:GetDamage()) > 0 then
			-- Skip karma and logging for spectator prop damage
			local isSpectatorDamage = dmginfo:IsDamageType(DMG_CRUSH) and PROPSPEC.IsSpectatorDmg(dmginfo)
			if IsValid(isSpectatorDamage) then
				return
			end
