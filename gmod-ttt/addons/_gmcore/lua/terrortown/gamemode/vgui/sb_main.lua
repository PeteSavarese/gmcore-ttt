...
---@param name string Unique group name (becomes global GROUP_<name>)
...
---@param p Player The player to classify
---@return number group GROUP_* constant
...
function PANEL:Init()
	...
	self.mapchange.Think = function (sf)
														sf:SetText("Round " .. math.Clamp(GetConVar("ttt_round_limit"):GetInt() - GetGlobalInt("ttt_rounds_left", 6) + 1, 1, GetConVar("ttt_round_limit"):GetInt()) .. "/" .. GetConVar("ttt_round_limit"):GetInt())
														sf:SizeToContents()
													end
...
---@param _ any Unused (kept for API compat)
---@param width? number Column width in pixels (default 50)
---@param sort_id? string Identifier for sorting
---@param sort_func? fun(a: Player, b: Player): number Comparator returning >0 if a should sort after b
---@return DLabel lbl The created label
...
---@param _ any Unused
---@param width? number Header width in pixels (default 50)
---@param sort_id? string Sorting identifier
---@param sort_func? fun(a: Player, b: Player): number Comparator
---@return DLabel lbl The created label
...
--   gui.EnableScreenClicker(scrolling)
	...
	--print("Entered loop in sb_main.lua:L327")
	while tw > hw do
			hname = string.sub(hname, 1, -6) .. "..."
			tw, th = surface.GetTextSize(hname)
	end
	--print("Exited loop in sb_main.lua:L327")
	...
	local cx = w - 8 -(scrolling and 16 or 0) - 25
