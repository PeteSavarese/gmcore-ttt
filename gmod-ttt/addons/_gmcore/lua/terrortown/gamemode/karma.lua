---Integrates fun-round bypasses, staff-specific kick thresholds,
---and chat notifications for karma bans.
...
KARMA.cv.staffkicklevel = CreateConVar("ttt_karma_low_amount_staff", "600")
KARMA.cv.autoban   =      CreateConVar("ttt_karma_low_ban", "1")
KARMA.cv.bantime   =      CreateConVar("ttt_karma_low_ban_minutes", "60")


util.AddNetworkString("gmcore.Karma.SendChatMessage")
...
---@param victim_karma number The victim's current karma
---@param dmg number Damage dealt
---@return number penalty Karma penalty to apply to the attacker
...
---@param victim_karma number The victim's current karma
---@return number penalty Karma penalty for a kill
...
---@param dmg number Damage dealt to the traitor
---@return number reward Karma reward
...
---@return number reward Karma reward for killing a traitor
function KARMA.GetKillReward()
	return KARMA.GetHurtReward(config.tbonus:GetFloat())
end

---@param ply Player Attacker receiving the penalty
---@param penalty number Amount of karma to deduct
---@param victim Player The player who was hurt/killed
function KARMA.GivePenalty(ply, penalty, victim)
	if !hook.Call("TTTKarmaGivePenalty", nil, ply, penalty, victim) then
		ply:SetLiveKarma(math.max(ply:GetLiveKarma() - penalty, 0))
	end
end

---@param ply Player Player receiving the reward
---@param reward number Base reward amount (before decay)
---@return number? reward Actual reward given (after decay/cap), or nil if blocked
function KARMA.GiveReward(ply, reward)
	reward = KARMA.DecayedMultiplier(ply) * reward

	if hook.Run("TTTKarmaGiveReward", ply, reward) == false then return end
	ply:SetLiveKarma(math.min(ply:GetLiveKarma() + reward, config.max:GetFloat()))

	return reward
end

---@param ply Player Player whose damage factor to set based on karma
...
---@param attacker Player The player who dealt damage
---@param victim Player The player who received damage
---@param dmginfo CTakeDamageInfo Damage information
function KARMA.Hurt(attacker, victim, dmginfo)
	...
	if victim.isPoltergeisted then return end
	...
	elseif (!attacker:GetTraitor()) and victim:GetTraitor() then
...
---@param attacker Player The player who dealt the killing blow
...
---@param ply Player Player whose karma determines the decay multiplier
---@return number multiplier Value between 0 and 1 used to scale karma rewards
...
function KARMA.RoundIncrement()
	...
	for _, ply in player.Iterator() do
		if ply:IsDeadTerror() and ply.death_type != KILL_SUICIDE or !ply:IsSpec() then
...
---@param ply Player Player to send karma notification to
...
---@param ply Player Player to initialise karma for
...
---@param ply Player Player whose karma to persist
function KARMA.Remember(ply)
	if ply.karma_kicked or (!ply:IsFullyAuthenticated()) then return end
...
---@param ply Player Player to recall stored karma for
---@return number? karma Previously stored karma, or nil if not found
function KARMA.Recall(ply)
	if config.persist:GetBool() then
		ply.delay_karma_recall = !ply:IsFullyAuthenticated()
...
---@param ply Player Player to check for auto-kick/ban
function KARMA.CheckAutoKick(ply)
	if (ply:HasStaffPerms() and ply:GetBaseKarma() < config.staffkicklevel:GetInt()) or (!ply:HasStaffPerms() and ply:GetBaseKarma() < config.kicklevel:GetInt()) then
		local msg = ply:Nick() .. "(" .. ply:SteamID64() .. ") was karma banned."

		for _, p in ipairs(player.GetHumans()) do
			if p and p != ply then
				net.Start("gmcore.Karma.SendChatMessage")
				net.WriteString(msg)
				net.Send(p)
			end
		end
...
---@param printfn fun(msg: string) Function used to output each line
