...
local Equipment = nil
---@param role number The role constant (e.g. ROLE_TRAITOR, ROLE_DETECTIVE)
---@return table equipment List of available equipment entries for that role
...
---@param item table Equipment entry table
---@return boolean isWeapon True if the equipment item is a weapon rather than a passive item
local function ItemIsWeapon(item) return not tonumber(item.id) end
---@param item table Equipment entry table with a `kind` field
---@return boolean canCarry True if the local player can carry this weapon type
...
---@param parent Panel The VGUI parent panel
---@param x number X offset
---@param y number Y offset
---@return fun(selected: table): boolean Validator function - returns true if purchase is allowed
...
gmcore.TraitorWarnings = gmcore.TraitorWarnings or {}
local tw = gmcore.TraitorWarnings

function GM:OnContextMenuOpen()
	local r = GetRoundState()
	if r == ROUND_ACTIVE then
			if GetConVarNumber("gmcore_traitor_warning_radial") > 0 then
				tw:Show()
				tw.closed = false
			else
				if (LocalPlayer():GetTraitor() or LocalPlayer():GetDetective()) then
						if IsValid(eqframe) then
							eqframe:Close()
						else
...
function GM:OnContextMenuClose()
	if tw.menu and tw.menu:IsVisible() then
			if GetConVarNumber("gmcore_traitor_fastwarn") > 0 then
				tw.menu.dRadial:DoClick()
			end
	end

	tw:Close()
	if tw.closed == false then
			if IsValid(eqframe) then
				eqframe:Close()
			else
				RunConsoleCommand("ttt_cl_traitorpopup")
