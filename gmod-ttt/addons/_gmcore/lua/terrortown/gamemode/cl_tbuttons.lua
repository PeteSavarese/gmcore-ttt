---Adds a team-warn keybind (+walk) so traitors can alert teammates
---before activating a traitor button.
...
---@class TBHUD
---@field buttons table<number, Entity> Cached traitor button entities keyed by EntIndex
---@field buttons_count number Number of cached traitor buttons on the map
---@field focus_ent Entity|nil The currently focused traitor button
---@field focus_stick number CurTime() until which the focus sticks
TBHUD = {}
TBHUD.buttons = {}
...
---Rebuilds the cached list of traitor buttons on the map.
...
---@return boolean focused True if the local traitor is focused on a button
function TBHUD:PlayerIsFocused()
	return IsValid(LocalPlayer()) and LocalPlayer():IsActiveTraitor() and IsValid(self.focus_ent)
end

---@return boolean used True if a focused button was activated
...
---Sends a team chat warning about the focused traitor button (GL custom).
---@return boolean warned True if a warning was sent
function TBHUD:WarnFocused()
	if IsValid(self.focus_ent) and self.focus_stick >= CurTime() then
			RunConsoleCommand("ttt_warn_tbutton", tostring(self.focus_ent:EntIndex()))
			RunConsoleCommand("say_team", string.format("I am activating [%s] in 3 seconds!", self.focus_ent:GetDescription()))
	...
	for k, but in pairs(self.buttons) do
			...
			if not IsValid(focus_ent) and d > focus_d and (self.focus_stick < CurTime() or but == self.focus_ent) then
				...
				if (x < focus_range and y < focus_range and
						...
						y = y + 12
						surface.SetTextPos(x, y)
						surface.DrawText(string.format("Press %s to warn others traitors", warn_key))
						-- GL END
...
hook.Add("PlayerBindPress", "gmcore.TButtons.WarnBind", function(ply, bind, pressed)
	if not IsValid(ply) then return end

	if pressed and bind == "+walk" then
			if TBHUD:PlayerIsFocused() then
				return TBHUD:WarnFocused()
			end
	end
end)
