---Server-side XGUI module that registers replicated writable cvars for TTT.
---Exposes every TTT convar (round timing, haste mode, traitor/detective ratios,
---DNA, voice drain, karma, credits, prop possession, admin settings, and misc)
---as `ULib.replicatedWritableCvar` entries so the client XGUI panel can read
---and modify them in real time.
