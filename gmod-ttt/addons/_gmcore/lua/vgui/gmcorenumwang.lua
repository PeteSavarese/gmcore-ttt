---@class gmcoreNumberWang : GLTextInput
local PANEL = {}
...
function PANEL:Init()
	...
	self:GetInputPanel():SetUpdateOnType(true)
	self:GetInputPanel():SetNumeric(true)
...
function PANEL:SetValue(val)
	...
	if !self:HasFocus() then
		self:GetInputPanel():SetValue(tostring(val))
		self:GetInputPanel():ConVarChanged(val)
	end
...
function PANEL:GetValue()
	return tonumber(self:GetInputPanel():GetValue()) or 0
end
...
derma.DefineControl("GmcoreNumberWang", "Menu Option Line", PANEL, "GmcoreTextInput")
