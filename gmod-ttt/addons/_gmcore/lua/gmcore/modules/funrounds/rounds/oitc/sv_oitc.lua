---@class OITCEvent : FunRound
---@field KillsThisRound table<Player, number>
---@field OverrideWin fun(self: OITCEvent): number
---@field OverrideRoles fun(self: OITCEvent): boolean
---@field PlayerDeath fun(self: OITCEvent, victim: Player, wep: Weapon, attacker: Player)
---@field PreventWepPickup fun(self: OITCEvent, ply: Player, wep: Weapon): boolean
---@field DoPlayerDeath fun(self: OITCEvent, ply: Player, attacker: Entity, dmginfo: CTakeDamageInfo)
---@field PlayerAmmoChanged fun(self: OITCEvent, ply: Player, ammoID: number, oldCount: number, newCount: number)

local EVENT = gmcore.FunRounds.RegisteredFunRounds["oitc"] or {}
---@cast EVENT OITCEvent
...
function EVENT:Prepare()
	...
	self:AddHook("PlayerAmmoChanged", self.PlayerAmmoChanged)
end

local function supplyWeapons(ply)
	if ply:IsAlive() then
		ply:StripWeapons()
		local deagleSWEP = ply:Give("weapon_zm_revolver")
		deagleSWEP.AllowDrop = false
		deagleSWEP.Primary.Damage = 2000
		deagleSWEP.Primary.ClipSize = 1
		deagleSWEP.Primary.ClipMax = 0
		deagleSWEP.Primary.DefaultClip = 1

		deagleSWEP:SetClip1(1)

		local crowbarSWEP = ply:Give("weapon_zm_improvised")
		crowbarSWEP.Primary.Damage = 2000
		crowbarSWEP.Deploy = function() end -- Prevent fast crowbar upgrade from changing upon deploy
		crowbarSWEP.Primary.Delay = 0.5
...
function EVENT:PreventWepPickup(ply, wep)
	return wep:GetClass() == "weapon_zm_revolver" or wep:GetClass() == "weapon_zm_improvised"
end

function EVENT:PlayerAmmoChanged(ply, ammoID, oldCount, newCount)
	if(newCount != 0) then
		ply:SetAmmo( 0 , 12 )
	end
end
...
function EVENT:DoPlayerDeath(victim, attacker, dmginfo)
	...
	local attackerWep = attacker:GetWeapon("weapon_zm_revolver")
	attackerWep:SetClip1(attackerWep:Clip1() + 1)
end
