---Overrides default TTT voice, radio, and chat.
-- Voicechat popup, radio commands, text chat stuff
hook.Add("PostGamemodeLoaded", "gmcore.Modules.Voice.OverrideDefault", function()
	...
	function GAMEMODE:ChatText(idx, name, text, type)
	...
	---@param ply Player Player who sent the chat message
	---@param text string Chat message text
	---@param teamchat boolean Whether the message is team-only chat
	---@param dead boolean Whether the player is dead
	function GAMEMODE:OnPlayerChat(ply, text, teamchat, dead)
		if !IsValid(ply) then return BaseClass.OnPlayerChat(self, ply, text, teamchat, dead) end
		...
		if team and !dead then
			dead = true
		end

		if teamchat and ((!team and !ply:IsSpecial()) or team) then
			teamchat = false
		end
	...
	---@param slotidx number Radio command slot index to send
...
function RADIO:GetTargetType()
	...
	if !trace or (!trace.Hit) or (!IsValid(trace.Entity)) then return end
	...
	elseif ent:GetClass() == "prop_ragdoll" and CORPSE.GetPlayerNick(ent, "") ~= "" then
		if DetectiveMode() and !CORPSE.GetFound(ent, false) then
	...
	---@param target Player|Entity|string|nil Radio target to convert to a display string
---@return string printable The human-readable display name of the radio target
...
local function RadioMsgRecv()
	...
	if !(IsValid(sender) and sender:IsPlayer()) then return end
	...
	-- else it's a nickname
		...
		function GAMEMODE:PlayerSentRadioCommand(ply, name, target)
		...
		local function VoiceNotifyThink(pnl)
			if !(IsValid(pnl) and LocalPlayer() and IsValid(pnl.ply)) then return end
			if !(GetGlobalBool("ttt_locational_voice", false) and (!pnl.ply:IsSpec()) and (pnl.ply != LocalPlayer())) then return end
		...
		---@param ply Player Player who started voice chat
		function GAMEMODE:PlayerStartVoice(ply)
			local client = LocalPlayer()
			if !IsValid(g_VoicePanelList) or !IsValid(client) then return end
			...
			if client == ply then
				if client:IsActiveTraitor() then
					if (!client:KeyDown(IN_SPEED)) and (!client:KeyDownLast(IN_SPEED)) then
		...
		local function ReceiveVoiceState()
			...
			if (!IsValid(LocalPlayer())) or (!LocalPlayer():IsActiveTraitor()) then return end
		...
		local function VoiceClean()
			for ply, pnl in pairs(PlayerVoicePanels) do
				if (!IsValid(pnl)) or (!IsValid(ply)) then
		...
		---@param ply Player Player who stopped voice chat
		---@param no_reset? boolean
		function GAMEMODE:PlayerEndVoice(ply, no_reset)
			if IsValid(PlayerVoicePanels[ply]) then
				PlayerVoicePanels[ply]:Remove()
				PlayerVoicePanels[ply] = nil
			end

			if IsValid(ply) and !no_reset then
		...
		local function GetDrainRate()
			if !GetGlobalBool("ttt_voice_drain", false) then return 0 end
			if GetRoundState() != ROUND_ACTIVE then return 0 end
			local ply = LocalPlayer()
			if (!IsValid(ply)) or ply:IsSpec() then return 0 end

			local countActive = 0

			for _, v in ipairs(player.GetAll()) do
				if !v:GetNWBool("TTTIsForceSpec") then
					countActive = countActive + 1
				end
			end

			if countActive <= 8 then return 0 end -- Less than 8 players has no voice drain

			if ply:HasStaffPerms() then
				return 0
			elseif ply:IsStoreRank() then
				return gmcore.StoreRank.Ranks[ply:GetStoreRank()].voiceBattery
			else
		...
		local function IsTraitorChatting(client)
			return client:IsActiveTraitor() and (!client.traitor_gvoice)
		end

		function VOICE.Tick()
			if !GetGlobalBool("ttt_voice_drain", false) then return end
			local client = LocalPlayer()

			if VOICE.IsSpeaking() and (!IsTraitorChatting(client)) then
				client.voice_battery = client.voice_battery - GetDrainRate()

				if !VOICE.CanSpeak() then
		...
		---@param client Player Local player to draw the voice battery HUD for
