...
util.AddNetworkString("gmcore.Sprays.RemoveSpray")

---Removes various HTML/JS characters from the URL.
---@param url string URL to strip of dangerous characters
---@return string sanitizedURL The URL with dangerous characters removed
...
---Sanitizes and validates a player's spray URL, falling back to the default if invalid.
---@param url string Spray URL to sanitize and validate
---@param ply Player Player whose spray URL is being validated
---@return string validURL The validated spray URL, or the default URL if invalid
local function FixURL(url, ply)
	...
	if sprayType != SPRAYTYPE_INVALID then
		allowed = true
	end
...
---Removes a player's spray and broadcasts the removal to all clients.
---@param id64 string SteamID64
function spraymesh.RemoveSpray(id64)
	if !spraymesh.SPRAYDATA[id64] then return end
...
---Sends a player's spray to all clients.
---@param hitpos Vector Hit position on the sprayed surface
---@param hitnormal Vector Normal of the surface hit
---@param tracenormal Vector Direction of the spray trace
---@param ply Player Player who is spraying
...
hook.Add("PlayerSpray", "SprayMesh.OverrideNativeSpray", function(ply)
	...
	if !ply:IsTerror() then return end
	local id64 = ply:SteamID64()
	local sprayInfo = spraymesh.SPRAYDATA[id64] or {}

	-- The player must be alive, and the spray must not be on cooldown
	if (!sprayInfo.delay or sprayInfo.immediate) and ply:Alive() then
		-- Apply spray cooldown
		if !sprayInfo.immediate then
...
---Sends all existing sprays to a newly connected client.
---@param ply Player Newly connected player to send existing sprays to
...
--[[
	Given a player, broadcasts a net msg to all players to remove the spray from their client
]]
net.Receive("gmcore.Sprays.RemoveSpray", function(len, ply)
	if !ply:HasStaffPerms() then return end

	local plySteamId64 = net.ReadString()
	local affectedPly = player.GetBySteamID64(plySteamId64)

	spraymesh.RemoveSpray(plySteamId64)

	if IsValid(affectedPly) then
		affectedPly:SendLua([[RunConsoleCommand("SprayMesh_URL", "]] .. spraymesh.SPRAY_URL_DEFAULT .. [[")]])
		gmcore.chatprint(affectedPly, "[", Color(255, 0, 0), "NOTICE", color_white, "] Your spray was removed by ", Color(255, 0, 0), ply:Nick(), color_white, ". It may have violated our rules. Contact them for more info.")

		for _, v in player.Iterator() do
			if v:HasStaffPerms() then
				gmcore.chatprint(v, "[", Color(255, 0, 0), "NOTICE", color_white, "] ", Color(255, 0, 0), affectedPly:Nick(), color_white, "'s spray was removed by ", Color(255, 0, 0), ply:Nick(), color_white, ".")
			end
		end
	end
end)
