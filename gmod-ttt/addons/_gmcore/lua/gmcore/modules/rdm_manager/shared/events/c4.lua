...
function event:TTTC4Arm(ply, originalPlacer, bomb, time)
	event.CallEvent({
		[1] = 1,
		[2] = ply:Nick(),
		[3] = ply:GetRole(),
		[4] = ply:SteamID(),
		[5] = IsValid(originalPlacer) and originalPlacer:Nick() or TTTLogTranslate(GetDMGLogLang, "ChatDisconnected"),
		[6] = time
	})
end
...
function event:ToString(v)
	if v[1] == 1 then
		return string.format(TTTLogTranslate(GetDMGLogLang, "C4Armed") .. " for %s", v[2], Damagelog:StrRole(v[3]), v[5], util.SimpleTime(v[6], "%02i:%02i"))
	elseif v[1] == 2 then
...
function event:IsAllowed(tbl)
	return Damagelog:IsFilterEnabled("filter_show_c4")
end
