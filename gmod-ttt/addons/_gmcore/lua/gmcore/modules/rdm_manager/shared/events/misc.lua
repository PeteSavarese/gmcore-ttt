if SERVER then
	...
	Damagelog:EventHook("TTTTripminePlaced")
	Damagelog:EventHook("TTTDeathstationPlaced")
	Damagelog:EventHook("Initialize") -- Used to override teleporter to prevent spamming damagelogs
	Damagelog:EventHook("glPointshopPlayerBoughtRole")
else
	Damagelog:AddFilter("filter_show_disguises", DAMAGELOG_FILTER_BOOL, true)
...
function event:glPointshopPlayerBoughtRole(ply)
	if not IsValid(ply) or not ply:Alive() then return end

	-- Add timer since this hook is called before the round is considered active and CallEvent has
	-- if check to see if round is active or not, causing this event to not be logged
...
function event:TTTDeathstationPlaced(ply)
	self.CallEvent({
		[1] = 6,
		[2] = ply:GetDamagelogID()
	})
...
function event:TTTTripminePlaced(ply)
	self.CallEvent({
		[1] = 7,
		[2] = ply:GetDamagelogID()
	})
...
function event:ToString(v, roles)
	...
	elseif v[1] == 5 then
		return string.format(TTTLogTranslate(GetDMGLogLang, "RoundBought"), ply.nick, Damagelog:StrRole(ply.role))
	elseif v[1] == 6 then
		return string.format(TTTLogTranslate(GetDMGLogLang, "DeathstationPlaced"), ply.nick, Damagelog:StrRole(ply.role))
	elseif v[1] == 7 then
		return string.format(TTTLogTranslate(GetDMGLogLang, "TripminePlaced"), ply.nick, Damagelog:StrRole(ply.role))
	end
end

function event:IsAllowed(tbl)
	if (tbl[1] == 1 or tbl[1] == 3) and not Damagelog:IsFilterEnabled("filter_show_disguises") then
		return false
	end

	if tbl[1] == 2 and not Damagelog:IsFilterEnabled("filter_show_teleports") then
		return false
	end

	if tbl[1] == 4 and not Damagelog:IsFilterEnabled("filter_show_traps") then
		return false
	end

	if tbl[1] == 5 and not Damagelog:IsFilterEnabled("filter_show_psroles") then
		return false
	end
