if SERVER then
	...
	Damagelog:AddFilter("filter_show_radiocommands", DAMAGELOG_FILTER_BOOL, true)
	Damagelog:AddFilter("filter_show_radiokos", DAMAGELOG_FILTER_BOOL, true)
...
function event:ToString(v, roles)
	...
	local text = LANG.GetPTranslation(v[2], {
		player = param
	})
...
function event:IsAllowed(tbl, roles)
	...
	if tbl[2] ~= "quick_traitor" then
		return Damagelog:IsFilterEnabled("filter_show_radiocommands")
	else
		return Damagelog:IsFilterEnabled("filter_show_radiokos")
	end
end
