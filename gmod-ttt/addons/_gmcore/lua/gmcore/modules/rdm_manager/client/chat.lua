...
net.Receive("DL_BroadcastMessage", function()
	...
	if not id or not IsValid(ply) or not ply:IsPlayer() or not color or not message then
		return
	end
...
net.Receive("DL_OpenChat", function()
	...
	if not report or not IsValid(admin) or not admin:IsPlayer() or not IsValid(victim) or not victim:IsPlayer() or not IsValid(attacker) or not attacker:IsPlayer() then
		return
	end
...
net.Receive("DL_JoinChatCL", function()
	...
	if is_joining then
		...
		if not chat or not IsValid(ply) or not ply:IsPlayer() then
			return
		end
...
net.Receive("DL_StopChat", function()
	...
	if not forced then
		...
		if not IsValid(admin) or not admin:IsPlayer() then
			return
		end
...
net.Receive("DL_LeaveChatCL", function()
	...
	if not IsValid(leaver) or not leaver:IsPlayer() then
		return
	end
