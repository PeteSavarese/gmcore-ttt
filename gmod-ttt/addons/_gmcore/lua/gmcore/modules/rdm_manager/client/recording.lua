...
hook.Add("Initialize", "GetSpecDMHook", function()
	...
	if tbl then ttt_specdm_hook = tbl.Think_Ghost end
end)
...
local function CreateShotsPanel()
	...
	Close:SetSize(20, 20)
	Close:SetPos(w - 25, 5)
	Close.DoClick = function() Frame:SetVisible(false) end
	...
	Frame.UpdateProgress = function(Frame, progress) Info:UpdateProgress(progress) end
...
if Damagelog.DeathSceneInitialized then Frame = CreateShotsPanel() end
cvars.AddChangeCallback("ttt_dmglogs_ds_showothers", function() if IsValid(Frame) then Frame:SetInfo() end end)
function Damagelog:CreateDSPanel()
	...
	showShots.DoClick = function() Frame:SetVisible(not Frame:IsVisible()) end
	...
	stop:SetSize(20, 20)
	stop:SetPos(w - 25, 5)
	stop.DoClick = function() self:StopRecording() end
end
...
net.Receive("DL_SendDeathScene", function()
	...
	if IsValid(Damagelog.Menu) then Damagelog.Menu:SetVisible(false) end
	Frame:SetInfo()
	paused = false
end)
		...
		if IsValid(v) then
			...
			if v.traces then
				...
				for _, v2 in pairs(v.traces) do
					if v2 then render.DrawBeam(v2[1], v2[2], 2, 0, 0, color) end
				end
			end
...
hook.Add("Think", "Think_Record", function()
	...
	if current_scene and (not last_curtime or (last_curtime and (CurTime() - last_curtime) >= 0.01)) then
		...
		Damagelog.DS_Replay.DoClick = function() i = 1 end
		if i < 1 then i = 1 end
		local progress = #current_scene - (#current_scene - i)
		if progress > #current_scene then progress = #current_scene end
		...
		if scene then
			...
			if not paused and Damagelog.DS_Play.Icon:GetImage() == "icon16/control_pause_blue.png" then Damagelog.DS_Play:DoClick() end
		end
		...
		for k, v in pairs(scene or {}) do
			if v.prop then
				if not props[k] then props[k] = ClientsideModel(v.model or "", RENDERGROUP_TRANSLUCENT) end
				local vector = v.pos
				local angle = v.ang
				...
				if models[k] and v.corpse and not models[k].corpse then
					if IsValid(models[k]) then models[k]:Remove() end
					models[k] = nil
				end
				...
				if v.corpse then
					...
					models[k].wep = v.wep
					models[k].hp = v.hp
					if models[k].SetSequence then models[k]:SetSequence(v.sequence) end
					if paused then
		...
		if scene and playedsounds ~= scene then
			for k, v in pairs(scene) do
				...
				if v.trace then
					if not models[k].traces then models[k].traces = {} end
					local index = table.insert(models[k].traces, v.trace)
					timer.Simple(0.2, function() if models[k] and models[k].traces then models[k].traces[index] = false end end)
				end
			end
...
function Damagelog:StopRecording()
	...
	i = 1
	playedsounds = {}
	if ttt_specdm_hook then hook.Add("Think", "Think_Ghost", ttt_specdm_hook) end
	for _, v in ipairs(player.GetHumans()) do
		v:SetNoDraw(false)
	end

	gui.EnableScreenClicker(false)
	if IsValid(self.Menu) then self.Menu:SetVisible(true) end
	if IsValid(Frame) then Frame:SetVisible(false) end
end

hook.Add("OnContextMenuOpen", "Recording", function() if current_scene then return false end end)
