...
---@param text string Text string to render
---@param font string Font name to use for rendering
---@param x number Horizontal position in pixels
---@param y number Vertical position in pixels
---@param color Color Text color including alpha
---@param align? number
...
---@param report table RDM report data to display in the report frame
...
---@param found Player|nil Attacker found in damage logs
---@param deathLogs table Damage log entries
---@param previousReports table[] Previous reports
---@param currentReports table[] Current reports
---@param dnas table DNA evidence
function Damagelog:ReportWindow(found, deathLogs, previousReports, currentReports, dnas)
	...
	Type.OnSelect = function(Type, data, text)
		...
		elseif not Entry:IsEnabled() then
	...
	icon:SetPos(13 + statusW, 13)
	local curStatus = vgui.Create("DLabel", self.ReportsInfo)
	curStatus:SetTextColor(color_white)
	...
	if #previousReports > 0 then
		...
		form:SetLabel(TTTLogTranslate(GetDMGLogLang, "PreviousMapReports"))
		form:SetPadding(0)
		form:SetSpacing(0)
	...
	if table.Count(currentReports) > 0 then
		...
		for _, key in ipairs(order) do
			...
			roundForm:SetLabel(TTTLogTranslate(GetDMGLogLang, "Round") .. " " .. key)
...
hook.Add("HUDPaint", "DamagelogPendingReports", function()
	...
	if not client:CanUseRDMManager() or not showPending:GetBool() then return end
