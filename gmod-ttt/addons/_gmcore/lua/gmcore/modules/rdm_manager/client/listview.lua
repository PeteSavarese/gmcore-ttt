	...
	item.OnRightClick = function()
		...
		if item.DeathScene then
			menu:AddOption(TTTLogTranslate(GetDMGLogLang, "ShowDeathScene"), function()
				if not item.sceneid then
					GMCore_MessageDialog(TTTLogTranslate(GetDMGLogLang, "NoDeathSceneId"), "Error", "Close")
					return
				end
...
local shoot_colors = {
	...
	[Color(220, 220, 220)] = true,
	[Color(255, 215, 0)] = true,
	[Color(255, 165, 0)] = true,
	[Color(255, 99, 71)] = true,
	[Color(255, 105, 180)] = true,
	[Color(255, 0, 255)] = true,
	[Color(0, 255, 0)] = true,
	[Color(144, 238, 144)] = true,
	[Color(0, 255, 255)] = true,
	[Color(135, 206, 235)] = true,
	[Color(0, 150, 255)] = true,
	[Color(189, 96, 255)] = true,
	[Color(255, 0, 162)] = true,
	[Color(255, 140, 0)] = true,
	[Color(255, 228, 181)] = true,
	[Color(173, 216, 230)] = true,
	[Color(240, 230, 140)] = true,
	[Color(211, 84, 0)] = true,
	[Color(151, 133, 255)] = true,
}
		...
		for _, v in ipairs(nums) do
			...
			for _, i in pairs(info) do
				...
				item.PaintOver = function()
					...
					if Damagelog.Highlighted and #Damagelog.Highlighted > 0 then
						should_highlight = table.HasValue(Damagelog.Highlighted, i[1])
						-- Also check target player if it exists (for actions with targets like crowbar pushes)
						if not should_highlight and i[3] then
							should_highlight = table.HasValue(Damagelog.Highlighted, i[3])
						end
					end

					if should_highlight then
						-- Use the same highlighting background color as damage logs
						surface.SetDrawColor(255, 0, 0, 100)
						surface.DrawRect(0, 0, item:GetWide(), item:GetTall())
					end
