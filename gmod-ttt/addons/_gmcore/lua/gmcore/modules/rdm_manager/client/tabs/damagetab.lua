...
function Damagelog:DrawDamageTab(x, y)
	...
	for k in pairs(Damagelog.filters) do
		local value = Damagelog:IsFilterEnabled(k)

		local option = filters:AddOption(TTTLogTranslate(GetDMGLogLang, k), function()
		if Damagelog:IsFilterEnabled(k) then
			Damagelog.disabled_filters[k] = true
		else
			Damagelog.disabled_filters[k] = nil
		end
	...
	self.PS_Label:SetTextColor(color_white)
	...
	local contentRowWidth = menuW - 75
	self.RoundPanel:SetWide(contentRowWidth)
	local buttonsW = self.Filters:GetWide()
	if IsValid(self.FullDeathSceneBtn) then
		buttonsW = buttonsW + 5 + self.FullDeathSceneBtn:GetWide()
	end
	local roundW = contentRowWidth - buttonsW - 5
	if roundW < 150 then roundW = 150 end
	self.Round:SetSize(roundW, 22)
	if IsValid(self.FullDeathSceneBtn) then
		self.Filters:SetPos(contentRowWidth - self.Filters:GetWide(), 0)
		self.FullDeathSceneBtn:SetPos(self.Filters.x - 5 - self.FullDeathSceneBtn:GetWide(), 0)
	else
		self.Filters:SetPos(contentRowWidth - self.Filters:GetWide(), 0)
	end
	self.PlayerSelect:SetSize(contentRowWidth, 60)
	local comboExtra = 100
	local comboW = contentRowWidth - comboExtra
	if comboW < 150 then comboW = contentRowWidth - 90 end
	self.PlayersCombo:SetSize(comboW, 20)
	self.Highlight:SetPos(self.PlayersCombo.x + comboW + 10, 30)
	local eventBase = contentRowWidth - 61
	local iconBase = 30
	if self.Damagelog.VBar and self.Damagelog.VBar.Enabled then
		self.Damagelog.EventColumn:SetFixedWidth(eventBase - 20)
		self.Damagelog.IconColumn:SetFixedWidth(iconBase + 20)
	else
		self.Damagelog.EventColumn:SetFixedWidth(eventBase)
		self.Damagelog.IconColumn:SetFixedWidth(iconBase)
	end
	end
