...
---@param status number RDM_MANAGER_* constant
---@return boolean isResolved True if the report status indicates it has been resolved
...
---@param steamid string SteamID to look up among connected players
---@return Player|nil player The connected player matching the SteamID, or nil if not found
...
---@return number remaining The number of reports the player can still submit this round
...
---@param report table RDM report data to send to the player
...
---@param ply Player Player to retrieve reports for
---@return table[] reports List of previous reports filed against the player
...
---@param ply Player Player initiating the RDM report
function Damagelog:StartReport(ply)
	...
	if gmcore.FunRounds.ActiveRound then
		ply:Damagelog_Notify(DAMAGELOG_NOTIFY_ALERT, "You can't report during a fun round!", 5, "buttons/weapon_cant_buy.wav")
...
---@param ply Player Reporting player
---@param attacker Player|string Reported player or SteamID
---@param message string Report message
---@param reportType number DAMAGELOG_REPORT_* constant
function HandlePlayerReport(ply, attacker, message, reportType)
	...
	for _, v in ipairs(player_GetHumans()) do
		if v:CanUseRDMManager() then
			if v:IsActive() then
				v:Damagelog_Notify(DAMAGELOG_NOTIFY_ALERT, TTTLogTranslate(v.DMGLogLang, "ReportCreated") .. " (#" .. index .. ") !", 5, "damagelogs/vote_failure.mp3")
			else
				if reportType ~= DAMAGELOG_REPORT_STANDARD then
					v:Damagelog_Notify(DAMAGELOG_NOTIFY_ALERT, string_format(TTTLogTranslate(v.DMGLogLang, "HasAdminReported"), ply:Nick(), attacker:Nick(), index), 5, "damagelogs/vote_failure.mp3")
				else
					v:Damagelog_Notify(DAMAGELOG_NOTIFY_ALERT, string_format(TTTLogTranslate(v.DMGLogLang, "HasReported"), ply:Nick(), attacker:Nick(), index), 5, "damagelogs/vote_failure.mp3")
				end
			end
	...
	if not attacker:CanUseRDMManager() then
		if reportType ~= DAMAGELOG_REPORT_STANDARD then
			attacker:Damagelog_Notify(DAMAGELOG_NOTIFY_ALERT, string_format(TTTLogTranslate(attacker.DMGLogLang, "HasAdminReportedYou"), ply:Nick()), 5, "damagelogs/vote_failure.mp3")
		else
			attacker:Damagelog_Notify(DAMAGELOG_NOTIFY_ALERT, string_format(TTTLogTranslate(attacker.DMGLogLang, "HasReportedYou"), ply:Nick()), 5, "damagelogs/vote_failure.mp3")
		end
	end
	...
	if IsValid(syncEnt) then
		syncEnt:CalculatePendingReports()
	end
...
local colorBlue = Color(0, 103, 196)
local colorGreen = Color(0, 255, 0)
local colorRed = Color(255, 0, 0)
local colorWhite = color_white or Color(255,255,255)
...
net.Receive("DL_UpdateStatus", function(_len, ply)
	...
	if previousStatus == status then return end

	-- set new status and admin (if not waiting)
	tbl.status = status
	tbl.admin = status == RDM_MANAGER_WAITING and false or ply:Nick()

	local victimPly = GetBySteamID(tbl.victim)
	local reportedPly = GetBySteamID(tbl.attacker)
	local msg
	...
	if status == RDM_MANAGER_WAITING then
		...
		if IsValid(victimPly) and gmcore and gmcore.chatprint then
			gmcore.chatprint(victimPly, colorBlue, ply:Nick(), colorWhite, " has unassigned themselves from your report against ", colorBlue, tbl.attacker_nick)
		end
		if IsValid(reportedPly) and gmcore and gmcore.chatprint then
			gmcore.chatprint(reportedPly, colorBlue, ply:Nick(), colorWhite, " has unassigned themselves from the report filed against you by ", colorBlue, tbl.victim_nick)
		end
	elseif status == RDM_MANAGER_PROGRESS then
		...
		for _, v in ipairs(player_GetHumans()) do
			if v:SteamID() == tbl.victim then
				v:Damagelog_Notify(DAMAGELOG_NOTIFY_INFO, ply:Nick() .. " " .. TTTLogTranslate(ply.DMGLogLang, "HandlingYourReport"), 5, "damagelogs/vote_yes.mp3")
			end
		end
		if IsValid(victimPly) and gmcore and gmcore.chatprint then
			gmcore.chatprint(victimPly, colorBlue, ply:Nick(), colorWhite, " is now investigating your report against ", colorBlue, tbl.attacker_nick)
		end
		if IsValid(reportedPly) and gmcore and gmcore.chatprint then
			gmcore.chatprint(reportedPly, colorBlue, ply:Nick(), colorWhite, " is now investigating the report filed against you by ", colorBlue, tbl.victim_nick)
		end
	elseif status == RDM_MANAGER_VALID then
		msg = string_format(TTTLogTranslate(ply.DMGLogLang, "HasSetReport"), ply:Nick(), index, TTTLogTranslate(ply.DMGLogLang, "Valid"))
		if IsValid(victimPly) and gmcore and gmcore.chatprint then
			gmcore.chatprint(victimPly, colorBlue, ply:Nick(), colorWhite, " has completed your report against ", colorBlue, tbl.attacker_nick, colorWhite, " and marked it as ", colorGreen, "valid.")
		end
		if IsValid(reportedPly) and gmcore and gmcore.chatprint then
			gmcore.chatprint(reportedPly, colorBlue, ply:Nick(), colorWhite, " has completed the report filed against you by ", colorBlue, tbl.victim_nick, colorWhite, " and marked it as ", colorGreen, "valid.")
		end
	elseif status == RDM_MANAGER_INVALID then
		msg = string_format(TTTLogTranslate(ply.DMGLogLang, "HasSetReport"), ply:Nick(), index, TTTLogTranslate(ply.DMGLogLang, "Invalid"))
		if IsValid(victimPly) and gmcore and gmcore.chatprint then
			gmcore.chatprint(victimPly, colorBlue, ply:Nick(), colorWhite, " has completed your report against ", colorBlue, tbl.attacker_nick, colorWhite, " and marked it as ", colorRed, "invalid.", colorWhite, " You may PM them for more info if you wish.")
		end
		if IsValid(reportedPly) and gmcore and gmcore.chatprint then
			gmcore.chatprint(reportedPly, colorBlue, ply:Nick(), colorWhite, " has completed the report filed against you by ", colorBlue, tbl.victim_nick, colorWhite, " and marked it as ", colorRed, "invalid.")
		end
	elseif status == RDM_MANAGER_INVALID_WVW then
		msg = string_format(TTTLogTranslate(ply.DMGLogLang, "HasSetReport"), ply:Nick(), index, TTTLogTranslate(ply.DMGLogLang, "InvalidWvW"))
		if IsValid(victimPly) and gmcore and gmcore.chatprint then
			gmcore.chatprint(victimPly, colorBlue, ply:Nick(), colorWhite, " has completed your report against ", colorBlue, tbl.attacker_nick, colorWhite, " and marked it as ", colorRed, "invalid (Word vs. Word).", colorWhite, " You may PM them for more info if you wish.")
		end
		if IsValid(reportedPly) and gmcore and gmcore.chatprint then
			gmcore.chatprint(reportedPly, colorBlue, ply:Nick(), colorWhite, " has completed the report filed against you by ", colorBlue, tbl.victim_nick, colorWhite, " and marked it as ", colorRed, "invalid (Word vs. Word).")
		end
	end
	...
	Damagelog:GetSyncEnt():CalculatePendingReports()
	Damagelog:SendLogToVictim(tbl)
	UpdatePreviousReports()

	if IsReportResolved(status) then
		local discordUpdate = {
			reportId = index,
			round = Damagelog.CurrentRound or 0,
			victim = {
...
net.Receive("DL_Conclusion", function(_len, ply)
	...
	if IsReportResolved(tbl.status) then
		local discordUpdate = {
			reportId = index,
			round = Damagelog.CurrentRound or 0,
			victim = {
...
function HandleReportedPlayerAnswer(ply, previous, text, index)
	...
	for _, v in ipairs(player_GetHumans()) do
		if v:CanUseRDMManager() then
			v:Damagelog_Notify(DAMAGELOG_NOTIFY_INFO, string_format(TTTLogTranslate(v.DMGLogLang, "HasAnsweredReport"), v:IsActive() and TTTLogTranslate(v.DMGLogLang, "TheReportedPlayer") or ply:Nick(), index), 5, "damagelogs/vote_yes.mp3")
			v:UpdateReport(previous, index)
		end
	end
...
net.Receive("DL_GetForgive", function(_, ply)
	...
	for _, v in ipairs(player_GetHumans()) do
		if v:CanUseRDMManager() then
			if forgive then
				if v:IsActive() then
					v:Damagelog_Notify(DAMAGELOG_NOTIFY_INFO, TTTLogTranslate(v.DMGLogLang, "TheReport") .. " #" .. index .. " " .. TTTLogTranslate(v.DMGLogLang, "HasCanceledByVictim"), 5, "damagelogs/vote_yes.mp3")
				else
					v:Damagelog_Notify(DAMAGELOG_NOTIFY_INFO, string_format(TTTLogTranslate(v.DMGLogLang, "HasCanceledReport"), ply:Nick(), index), 5, "damagelogs/vote_yes.mp3")
				end
			else
				if v:IsActive() then
					v:Damagelog_Notify(DAMAGELOG_NOTIFY_INFO, TTTLogTranslate(v.DMGLogLang, "NoMercy") .. " #" .. index .. " !", 5, "damagelogs/vote_yes.mp3")
				else
					v:Damagelog_Notify(DAMAGELOG_NOTIFY_INFO, string_format(TTTLogTranslate(v.DMGLogLang, "DidNotForgive"), ply:Nick(), tbl.attacker_nick, index), 5, "ui/vote_yes.mp3")
				end
			end
	...
	if forgive then
		ply:Damagelog_Notify(DAMAGELOG_NOTIFY_INFO, string_format(TTTLogTranslate(ply.DMGLogLang, "YouDecidedForgive"), tbl.attacker_nick), 5, "damagelogs/vote_yes.mp3")
	else
		ply:Damagelog_Notify(DAMAGELOG_NOTIFY_INFO, string_format(TTTLogTranslate(ply.DMGLogLang, "YouDecidedNotForgive"), tbl.attacker_nick), 5, "damagelogs/vote_no.mp3")
	end
	...
	if IsValid(attacker) then
		if forgive then
			attacker:Damagelog_Notify(DAMAGELOG_NOTIFY_INFO, string_format(TTTLogTranslate(attacker.DMGLogLang, "DecidedToForgiveYou"), ply:Nick()), 5, "damagelogs/vote_yes.mp3")
		else
			attacker:Damagelog_Notify(DAMAGELOG_NOTIFY_INFO, string_format(TTTLogTranslate(attacker.DMGLogLang, "DecidedNotToForgiveYou"), ply:Nick()), 5, "damagelogs/vote_no.mp3")
		end
	end
