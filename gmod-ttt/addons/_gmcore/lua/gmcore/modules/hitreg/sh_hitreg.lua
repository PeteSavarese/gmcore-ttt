---Clientside hitreg (CLHR) system. Intercepts EntityFireBullets on the client,
---sends hit reports to the server, and validates them against lag-compensated
---entity positions before applying damage.
---
---Ported from the marghitreg standalone addon into the GL module system.
---@class CLHR
---@field Exceptions table<string, boolean> Weapon classes excluded from clientside hitreg
---@field maxply_bits integer Bit width needed to encode max player count
---@field maxplayers integer Max player slot count (power of 2)
---@field hooks table<string|Entity, function> Captured EntityFireBullets hook handlers
---@field hookAdd function Original hook.Add before override
---@field hookRem function Original hook.Remove before override
---@field IgnoreBulletPlayer Player|nil Player whose bullets are being suppressed this tick
---@field IgnoreBulletTime number|nil CurTime() when bullet suppression was set
---@field PhysCollides table<string, PhysCollide|false> Cached physics collision meshes by model
---@field EntityFireBullets function Clientside bullet interception entry point
---@field PreEntityFireBullets function|nil Subtick shootpos correction (client only)
---@field SetupMove function|nil Hit application and air-duck tracking (server only)
CLHR = CLHR or {
	Exceptions = {
...
---Returns true if this entity's type category is enabled in clhr_targetbits.
---@param self Entity The entity to check
---@return boolean
...
---Returns the clhr_targetbits bit flag for this entity's type category.
---@param self Entity The entity to categorize
---@return integer
...
---Wraps `new` around `old` so both run when a bullet callback fires.
---If `old` is nil, returns `new` directly. `arg` is forwarded to `new` as a 4th argument.
---@param old function|nil Existing bullet callback, or nil if none
---@param new function Replacement callback to call first; receives (ply, trace, dmginfo, arg)
---@param arg any Extra argument forwarded to `new`
---@return function Combined callback
...
-- overriding the hook library functions is trash up
...
---Runs all captured EntityFireBullets hooks and the gamemode handler in order.
---Returns false if any hook returns false (suppressing the bullet), otherwise nil.
---@param ply Player The player firing the weapon
---@param data table The EntityFireBullets data table
---@return boolean|nil false if any hook suppressed the bullet, nil otherwise
