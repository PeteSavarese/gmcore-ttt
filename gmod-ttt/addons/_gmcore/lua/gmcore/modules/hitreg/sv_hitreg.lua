...
---Serverside EntityFireBullets intercept. Registers a per-bullet callback that
---records shot info and queues client-reported hits for lag-compensated validation.
---Called from the shared CLHR_EntityFireBullets hook when the firing player is local.
---@param ply Player The player firing
---@param data table EntityFireBullets data table
---@param wep Weapon|nil Active weapon at time of fire
---@param fixedshotgun integer|false|nil Shotgun fix mode (1=fot, 2=tttwr) or false/nil for single shot
---@param cmd integer Current user command number
---@param lastcmd integer Previous command number; duplicate commands are skipped
---@param tick integer TickCount from the user command, used to calculate ping
---@return boolean true to allow the bullet to proceed
...
---SetupMove hook handler. Tracks air-duck timing for hitbox tolerance adjustment,
---then validates all pending client hit reports against lag-compensated entity positions
---and dispatches damage for confirmed hits.
---@param ply Player The player whose movement is being set up
---@param mv CMoveData The move data for this tick
...
---Returns the cached PhysCollide mesh for a model, creating it if needed.
---For ragdolls, pass `num` (1-based physics object index) to get a specific convex mesh.
---Returns nil if the model has no valid physics mesh.
---@param mdl string Model path to look up
---@param num integer|nil Physics object index (1-based) for multi-part meshes (ragdolls)
---@return PhysCollide|nil
function CLHR.GetPhysCollides(mdl, num)
	---@type PhysCollide|PhysCollide[]|false|nil
