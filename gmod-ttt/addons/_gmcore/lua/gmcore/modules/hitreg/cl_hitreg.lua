...
---Clientside EntityFireBullets intercept. Registers a per-bullet callback that
---records hit traces and sends them to the server for validation.
...
---Subtick shootpos correction. Called before EntityFireBullets when `clhr_subtick` is enabled.
---If a shot was predicted in CreateMove at an earlier sub-tick position, rewrites `data.Src`
---and `data.Dir` to match that position so the server can validate the correct origin.
---@param ply Player The local player firing
---@param data table EntityFireBullets data table (Src and Dir may be modified)
