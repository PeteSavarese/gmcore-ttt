-- c4 explosive
GAMEMODE.ActiveC4 = {} -- Used for deathscenes
...
ENT.Type          = "anim"
ENT.DefaultModel  = Model("models/weapons/w_c4_planted.mdl")
...
function ENT:SetThrower(ply)
	...
	local isFirstPlace = self.thrower == nil

	self.thrower = ply

	if isFirstPlace and IsValid(ply) and SERVER then
		print("Setting thrower for the first time to " .. ply:Nick())
		-- Only set skin when initially placed. SetThrower is also called when armed so we don't
		-- want to reset the skin then.
...
function ENT:CheckActiveSkin(iteration)
	...
	if not IsValid(self:GetThrower()) then return end

	local owner = self:GetThrower()
	...
	for itemId, item in pairs(tPlyItems) do
		...
		if itemTbl.Class == "weapon_ttt_c4" then
			if itemTbl.WorldMaterials then
				self:SetModel(self.DefaultModel)

				for matIndex, mat in pairs(itemTbl.WorldMaterials) do
					self:SetSubMaterial(matIndex - 1, mat)
				end
			elseif itemTbl.WorldModel then
				self:SetModel(itemTbl.WorldModel)
			end

			-- Got init physics after model change in timery
...
function ENT:Initialize()
	...
	if ! table.HasValue(GAMEMODE.ActiveC4, self) then
		table.insert(GAMEMODE.ActiveC4, self)
	end
end
...
function ENT:WeldToGround(state)
	...
	if state then
		...
		local tr = util.TraceEntity(
			{ start = self:GetPos(), endpos = self:GetPos() - Vector(0, 0, 16), filter = ignore, mask = MASK_SOLID }, self)
...
function ENT:Explode(tr)
	...
	if SERVER then
		...
		if util.PointContents(pos) == CONTENTS_WATER or GetRoundState() != ROUND_ACTIVE then
			table.RemoveByValue(GAMEMODE.ActiveC4, self)
			self:Remove()
			self:SetExplodeTime(0)
			return
		end
...
if SERVER then
	...
	hook.Add("InitPostEntity", "gmcore.Entities.OverrideC4Commands", function()
		local function ReceiveC4Config(ply, cmd, args)
			if ! (IsValid(ply) && ply:IsTerror() && #args == 2) then return end
			local idx = tonumber(args[1])
			local time = tonumber(args[2])
			if ! idx || ! time then return end
			local bomb = ents.GetByIndex(idx)

			if IsValid(bomb) && bomb:GetClass() == "ttt_c4" && (! bomb:GetArmed()) then
				if bomb:GetPos():Distance(ply:GetPos()) > 256 then
					-- These cases should never arise in normal play, so no messages
					return
				elseif time < C4_MINIMUM_TIME || time > C4_MAXIMUM_TIME then
					return
				elseif IsValid(bomb:GetPhysicsObject()) && bomb:GetPhysicsObject():HasGameFlag(FVPHYSICS_PLAYER_HELD) then
					return
				else
					local originalPlacer = bomb:GetThrower() -- Have to log before bomb:Arm is called because the thrower and owner are changed to the ply who armed the C4

					LANG.Msg(ply, "c4_armed")
					bomb:Arm(ply, time)
					hook.Run("TTTC4Arm", ply, originalPlacer, bomb, time)
				end
			end
		end
		...
		local function SendDisarmResult(ply, bomb, result)
			hook.Run("TTTC4Disarm", ply, result, bomb)
		...
		local function ReceiveC4Disarm(ply, cmd, args)
			...
			if ! idx || ! wire then return end
			local bomb = ents.GetByIndex(idx)

			if IsValid(bomb) && bomb:GetClass() == "ttt_c4" && ! bomb.DisarmCausedExplosion && bomb:GetArmed() then
				if bomb:GetPos():Distance(ply:GetPos()) > 256 then
					return
				elseif bomb.SafeWires[wire] || ply:IsTraitor() || ply == bomb:GetOwner() then
		...
		local function ReceiveC4Pickup(ply, cmd, args)
			if ! (IsValid(ply) && ply:IsTerror() && #args == 1) then return end
			local idx = tonumber(args[1])
			...
			if IsValid(bomb) && bomb:GetClass() == "ttt_c4" && (! bomb:GetArmed()) then
				if bomb:GetPos():Distance(ply:GetPos()) > 256 then
					return
				elseif ! ply:CanCarryType(WEAPON_EQUIP1) then
					LANG.Msg(ply, "c4_no_room")
				else
					local prints = bomb.fingerprints || {}
					hook.Run("TTTC4Pickup", ply, bomb)
					local wep = ply:Give("weapon_ttt_c4")

					if IsValid(wep) then
						wep.fingerprints = wep.fingerprints || {}
						table.Add(wep.fingerprints, prints)
						table.RemoveByValue(GAMEMODE.ActiveC4, bomb)
						bomb:Remove()
					end
				end
			end
		...
		local function ReceiveC4Destroy(ply, cmd, args)
			...
			if IsValid(bomb) && bomb:GetClass() == "ttt_c4" && (! bomb:GetArmed()) then
				if bomb:GetPos():Distance(ply:GetPos()) > 256 then
					...
					hook.Run("TTTC4Destroyed", ply, bomb)
	...
	function ENT:Draw()
		...
		if self:GetArmed() then
			...
			if angpos_ur then
				...
				draw.DrawText(strtime(max(0, self:GetExplodeTime() - CurTime()), "%02i:%02i"), "C4ModelTimer", -1, 1, COLOR_RED,
					TEXT_ALIGN_RIGHT)
				cam.End3D2D()
			end
		end
