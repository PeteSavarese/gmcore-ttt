...
function SWEP:PrimaryAttack()
	...
	if SERVER and tr.Hit and tr.HitNonWorld and IsValid(hitEnt) and hitEnt:IsPlayer() then
			...
			if hitEnt:Health() < (self.Primary.Damage + 10) or (self:GetOwner():PS_HasItemEquipped("upgrade_knife_instakill") and not gmcore.FunRounds.IsRoundFun) then
...
function SWEP:Deploy()
	if !self:GetOwner():PS_HasItemEquipped("upgrade_silent_knife") then
			self:EmitSound("weapons/knife/knife_deploy1.wav", 65, 100)
	end
end
