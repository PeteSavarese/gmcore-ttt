...
function SWEP:SecondaryAttack()
	...
	if tr.Hit and IsValid(tr.Entity) and tr.Entity:IsPlayer() and (self:GetOwner():EyePos() - tr.HitPos):Length() < 100 then
		...
		hook.Run("TTTCrowbarPush", self:GetOwner(), ply)
	end
...
function SWEP:Deploy()
	if self:GetOwner():PS_HasItemEquipped("upgrade_crowbar_draw") then
		local level = self:GetOwner():PS_GetUpgradeLevel("upgrade_crowbar_draw")

		if level > 0 then
			local speed = 0.5 * (0.8 - (0.1 * level))
			local damage = 20 - (2.7 * level)
