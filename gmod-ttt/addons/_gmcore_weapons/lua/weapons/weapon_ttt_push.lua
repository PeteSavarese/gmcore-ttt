...
function SWEP:FirePulse(force_fwd, force_up)
	...
	bullet.Callback = function(att, tr, dmginfo)
		...
		elseif SERVER and IsValid(ply) and not ply:IsPlayer() then
			PROPSPEC.Kick(ply)
		end
	end
	...
	if owner:PS_HasItemEquipped("upgrade_newton_jumper") then
			local pushvel = owner:GetAimVector():GetNegated() * fwd * 3
			pushvel.z = math.max(pushvel.z, up)

			owner:SetGroundEntity(nil)
			owner:SetLocalVelocity(owner:GetVelocity() + pushvel)
			owner.was_pushed = {att=owner, t=CurTime(), wep=self:GetClass()}
	end
end
