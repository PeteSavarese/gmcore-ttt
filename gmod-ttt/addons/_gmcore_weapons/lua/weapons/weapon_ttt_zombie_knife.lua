...
function SWEP:PrimaryAttack()
		...
		if SERVER then
			local screech = math.random(1, 3)
			self:GetOwner():SetAnimation( PLAYER_ATTACK1 )
			if screech == 3 and self:GetOwner():GetRole() == ROLE_TRAITOR then
				self:GetOwner():EmitSound("npc/fast_zombie/fz_frenzy1.wav")
			end
		end
...
function SWEP:StabKill(tr, spos, sdest)
		...
		timer.Simple(3, function() giveknife(ply) end)
...
function SWEP:SecondaryAttack()
		...
		if SERVER then
				...
				local knife = ents.Create("ttt_zombie_knife_proj")
				...
				local screech = math.random(1, 2)
				if screech == 2 and self:GetOwner():GetRole() == ROLE_TRAITOR then
...
--TODO: Add refresher function for knives on invalid drops.
function SWEP:OnDrop()
	if !self:IsValid() or !self:GetOwner() or !self:GetOwner():IsValid() then
		self:Remove()
		timer.Simple(1, function() refreshWeps() end)
		return
	end

	local ply = self:GetOwner()
	timer.Simple(2, function() giveknife(ply) end)
	self:Remove()
end
