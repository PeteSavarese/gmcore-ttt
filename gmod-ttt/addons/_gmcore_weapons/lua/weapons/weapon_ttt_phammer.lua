...
local function ValidTarget(ent, owner)
	if owner:PS_HasItemEquipped("upgrade_poltergeistable_players") and ent:IsPlayer() then return true end
	-- NOTE: cannot check for motion disabled on client

	return IsValid(ent) and ent:GetMoveType() == MOVETYPE_VPHYSICS and ent:GetPhysicsObject() and (!ent:IsWeapon()) and (!ent:GetNWBool("punched", false)) and (!ent:IsPlayer())
end
...
function SWEP:PrimaryAttack()
	...
	if SERVER then
		...
		if (tr.HitNonWorld and ValidTarget(tr.Entity, self:GetOwner()) and tr.Entity:GetPhysicsObject():IsMoveable()) or (self:GetOwner():PS_HasItemEquipped("upgrade_poltergeistable_players") and tr.Entity:IsPlayer()) then
			if tr.Entity:IsPlayer() and tr.Entity:IsGhost() then return end
...
function SWEP:SecondaryAttack()
	...
	if !(self:CanPrimaryAttack() and (self:GetNextPrimaryFire() - CurTime()) <= 0) then return end
	if IsValid(self.hammer) then return end
	if SERVER then
			local ply = self:GetOwner()
			if !IsValid(ply) then return end
			...
			if tr.HitNonWorld and ValidTarget(tr.Entity, self:GetOwner()) and tr.Entity:GetPhysicsObject():IsMoveable() then
...
function SWEP:CreateHammer(tgt, pos)
	...
	if IsValid(hammer) then
			...
			if !stuck then hammer:Remove() end
			self.hammer = hammer
	end
end
...
if SERVER then
	...
	function SWEP:Think()
			...
			if self.IsCharging and self:GetOwner():KeyDown(IN_ATTACK2) then
				...
				if tr.HitNonWorld and ValidTarget(tr.Entity, self:GetOwner()) then
...
if CLIENT then
	...
	function SWEP:ViewModelDrawn()
			...
			if muzzle_angpos == nil then return end -- added check to prevent nil value

			local spos = muzzle_angpos.Pos + muzzle_angpos.Ang:Forward() * 10 -- This is line 277
			...
			if plytr.HitNonWorld and ValidTarget(plytr.Entity, self:GetOwner()) then
				if d < maxrange then
					c = COLOR_GREEN
					a = 255
				else
