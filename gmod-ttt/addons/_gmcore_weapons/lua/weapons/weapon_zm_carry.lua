...
function SWEP:Reset(keep_velocity)
	if IsValid(self.CarryHack) then
			...
			hook.Run("TTTMagnetoDrop", ply, self.EntHolding)
	end
	...
	if IsValid(self.EntHolding) then
			...
			if IsValid(self.EntHolding) and IsValid(self:GetOwner()) then
				self.EntHolding:SetPhysicsAttacker(self:GetOwner())
			end

			if (!keep_velocity) and (no_throw:GetBool() or self.EntHolding:GetClass() == "prop_ragdoll") then
...
function SWEP:CheckValidity()
	if (!IsValid(self.EntHolding)) or (!IsValid(self.CarryHack)) or (!IsValid(self.Constr)) then
...
function SWEP:DoAttack(pickup)
	...
	if IsValid(self.EntHolding) then
		...
		if (!pickup) and self.EntHolding:GetClass() == "prop_ragdoll" then
			-- see if we can pin this ragdoll to a wall in front of us
			if !self:PinRagdoll() then
	...
	if IsValid(trace.Entity) then
		...
		if !IsValid(phys) or !phys:IsMoveable() or phys:HasGameFlag(FVPHYSICS_PLAYER_HELD) then return end
...
--[[
	Perform a pickup. Creates an invisible ghost ent and makes a constraint from the real prop to the ghost prop.
]]
function SWEP:Pickup()
	...
	if IsValid(ent) and IsValid(entphys) then
		...
		if IsValid(self.CarryHack) then
			...
			hook.Run("TTTMagnetoPickup", ply, self.EntHolding, self.CarryHack)
		end
	end
end
...
function SWEP:PinRagdoll()
	if !pin_rag:GetBool() then return end
	if (!self:GetOwner():IsTraitor()) and (!pin_rag_inno:GetBool()) then return end
	...
	if tr.HitWorld and (!tr.HitSky) then
			...
			if !IsValid(bonephys) then return end
...
if CLIENT then
	...
	function SWEP:DrawHUD()
		...
		if self.dt.can_rag_pin and IsValid(self.dt.carried_rag) then
			...
			if !client:IsSpec() and (self.dt.can_rag_pin_inno or client:IsTraitor()) then
