...
function IgniteTarget(att, path, dmginfo)
	...
	if SERVER then
			local level = IsValid(dmginfo:GetAttacker()) and dmginfo:GetAttacker():PS_HasItemEquipped("upgrade_enhanced_pyrotechnics") and dmginfo:GetAttacker():PS_GetUpgradeLevel("upgrade_enhanced_pyrotechnics") or 0
			local dur

			if level > 1 then
				dur = ent:IsPlayer() and 7 or 10
			else
				dur = ent:IsPlayer() and 5 + level or 10
			end
